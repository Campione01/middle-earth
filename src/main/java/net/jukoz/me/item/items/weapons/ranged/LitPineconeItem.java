...
public class LitPineconeItem extends PineconeItem{
    ...
    public LitPineconeItem(Properties settings) {
        super(settings);
    }
    ...
    public void releaseUsing(ItemStack stack, Level world, LivingEntity user, int remainingUseTicks) {
        ...
        if (!world.isClientSide) {
            ...
            litPineconeEntity.shootFromRotation(playerEntity, playerEntity.getXRot(), playerEntity.getYRot(), 0.0f, BASE_STRENGTH + (CHARGE_STRENGTH * percentage), 1.0f);

            world.addFreshEntity(litPineconeEntity);
            world.playSound(null, litPineconeEntity, SoundEvents.SNOWBALL_THROW, SoundSource.PLAYERS, 1.0f, 0.7f);

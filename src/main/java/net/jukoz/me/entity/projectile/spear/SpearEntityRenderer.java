...
public class SpearEntityRenderer<T extends SpearEntity> extends EntityRenderer<T> {
    ...
    public void render(T entity, float yaw, float tickDelta, PoseStack matrices, MultiBufferSource vertexConsumers, int light) {
        ...
        if(itemStack == null) entity.getDefaultPickupItem();
        this.itemRenderer.renderStatic(itemStack, ItemDisplayContext.THIRD_PERSON_RIGHT_HAND, light, OverlayTexture.NO_OVERLAY, matrices, vertexConsumers, entity.level(), entity.getId());
        matrices.popPose();

...
public class BoulderEntityRenderer extends EntityRenderer<BoulderEntity> {
    ...
    public void render(BoulderEntity entity, float yaw, float tickDelta, PoseStack matrices, MultiBufferSource vertexConsumers, int light) {
        ...
        this.blockRenderManager.getModelRenderer().tesselateBlock(world, this.blockRenderManager.getBlockModel(blockState), blockState, blockPos, matrices, vertexConsumers.getBuffer(ItemBlockRenderTypes.getMovingBlockRenderType(blockState)), false, RandomSource.create(), blockState.getSeed(entity.getOwner().blockPosition()), OverlayTexture.NO_OVERLAY);
        matrices.popPose();

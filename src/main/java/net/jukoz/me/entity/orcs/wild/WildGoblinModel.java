...
public class WildGoblinModel<T extends Mob>
        extends HumanoidModel<T> {

    public WildGoblinModel(ModelPart root) {
        super(root);
    }
    ...
    public void setupAnim(T mobEntity, float f, float g, float h, float i, float j) {
        ...
        WildGoblinEntity.State state = ((WildGoblinEntity)mobEntity).getState();

        if(state == WildGoblinEntity.State.ATTACKING) {
            // Walk added

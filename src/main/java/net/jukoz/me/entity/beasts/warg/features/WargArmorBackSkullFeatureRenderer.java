...
public class WargArmorBackSkullFeatureRenderer extends RenderLayer<WargEntity, WargModel> {
    private final WargArmorTopAddonsModel model;

    public WargArmorBackSkullFeatureRenderer(RenderLayerParent<WargEntity, WargModel> context, EntityModelSet loader) {
        super(context);
        this.model = new WargArmorTopAddonsModel(loader.bakeLayer(ModEntityModelLayers.WARG_ARMOR_ADDONS_BACK));
    }
    ...
    public void render(PoseStack matrices, MultiBufferSource vertexConsumers, int light, WargEntity wargEntity, float limbAngle, float limbDistance, float tickDelta, float animationProgress, float headYaw, float headPitch) {
        ...
        if(item instanceof CustomAnimalArmorItem animalArmorItem) {
            if (itemStack.is(ModEquipmentItems.WARG_GUNDABAD_PLATE_ARMOR)) {

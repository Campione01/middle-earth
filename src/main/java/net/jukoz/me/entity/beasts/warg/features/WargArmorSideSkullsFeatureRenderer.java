...
public class WargArmorSideSkullsFeatureRenderer extends RenderLayer<WargEntity, WargModel> {
    ...
    public WargArmorSideSkullsFeatureRenderer(RenderLayerParent<WargEntity, WargModel> context, EntityModelSet loader) {
        super(context);
        this.model = new WargArmorSideAddonsModel(loader.bakeLayer(ModEntityModelLayers.WARG_ARMOR_ADDONS_SIDE_SKULL));
    }
        ...
        if(item instanceof CustomAnimalArmorItem animalArmorItem) {
            if (animalArmorItem.getArmorType() == CustomAnimalArmorItem.Type.WARG) {
                if(itemStack.get(ModDataComponentTypes.MOUNT_ARMOR_DATA) != null && itemStack.get(ModDataComponentTypes.MOUNT_ARMOR_DATA).sideArmorAddon()) {
                    ...
                    ResourceLocation addonTexture = ResourceLocation.fromNamespaceAndPath(MiddleEarth.MOD_ID, "textures/entities/warg/feature/warg_armor_side_skull_addon.png");

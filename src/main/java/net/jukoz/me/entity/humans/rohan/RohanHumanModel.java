...
public class RohanHumanModel<T extends Mob>
        extends HumanoidModel<T> {

    public RohanHumanModel(ModelPart root) {
        super(root);
    }
    ...
    public void setupAnim(T mobEntity, float f, float g, float h, float i, float j) {
        ...
        RohanHumanEntity.State state = ((RohanHumanEntity)mobEntity).getState();

        if(state == RohanHumanEntity.State.ATTACKING) {
            // Walk added

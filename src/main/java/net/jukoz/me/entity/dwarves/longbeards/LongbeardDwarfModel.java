...
public class LongbeardDwarfModel<T extends Mob> extends HumanoidModel<T> {
    ...
    public void setupAnim(T mobEntity, float f, float g, float h, float i, float j) {
        ...
        DurinDwarfEntity.State state = ((DurinDwarfEntity)mobEntity).getState();

        if(state == DurinDwarfEntity.State.ATTACKING) {
            // Walk added

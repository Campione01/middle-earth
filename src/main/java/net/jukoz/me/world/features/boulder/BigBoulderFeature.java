        ...
        for(int x = (int) -forSize; x <= forSize; ++x) {
            for(int z = (int) -forSize; z <= forSize; ++z) {
                for(int y = (int) -baseHeight-3; y <= baseHeight+3; ++y) {
                    BlockState mutableBlockState = structureWorldAccess.getBlockState(blockPos.mutable().offset(x, y, z));
                    if(mutableBlockState.is(BlockTags.LOGS) || CANNOT_REPLACE_BLOCKS.contains(mutableBlockState.getBlock())) {
                        continue;

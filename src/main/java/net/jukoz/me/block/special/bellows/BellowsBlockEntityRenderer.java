...
public class BellowsBlockEntityRenderer implements BlockEntityRenderer<BellowsBlockEntity>  {
    ...
    public BellowsBlockEntityRenderer(BlockEntityRendererProvider.Context ctx) {
        ModelPart modelPart = ctx.bakeLayer(ModEntityModelLayers.BELLOWS);
    ...
    @Override
    public void render(BellowsBlockEntity bellowsBlockEntity, float tickDelta, PoseStack matrices, MultiBufferSource vertexConsumers, int light, int overlay) {
        Level world = bellowsBlockEntity.getLevel();
        BlockState blockState = world != null
                ? bellowsBlockEntity.getBlockState()
    ...
    private Material getSpriteIdentifier(float animationProgress){
        ...
        return new Material(TextureAtlas.LOCATION_BLOCKS,
                ResourceLocation.fromNamespaceAndPath(MiddleEarth.MOD_ID, "model/bellows/bellows_" + spriteState));
    }
}

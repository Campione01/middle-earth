...
public class ReinforcedChestEntityRenderer<T extends ChestBlockEntity> extends ChestRenderer<T> {
    ...
    public ReinforcedChestEntityRenderer(BlockEntityRendererProvider.Context ctx) {
        super(ctx);
        ModelPart modelPart = ctx.bakeLayer(ModEntityModelLayers.REINFORCED_CHEST);
    ...
    public void render(T entity, float tickDelta, PoseStack matrices, MultiBufferSource vertexConsumers, int light, int overlay) {
        Material spriteIdentifier = new Material(Sheets.CHEST_SHEET,
        ...
        float g = properties.apply(ReinforcedChestBlock.opennessCombiner(entity)).get(tickDelta);
        g = 1.0F - g;
        g = 1.0F - g * g * g;
